...
Outputs to ./templates/ and packages ./whatshername-kit.zip
"""
import json
import os
import secrets
...
def shop_item(url, title, meta, price, col=33):
    """Framed product card with an Add-to-Cart link (static scaffold —
    becomes a WooCommerce Products widget once Woo is installed)."""
    return column([framed(url, title.split()[0])] +
                  caption(title, meta, price) +
                  [spacer(14), link_button("Add to Cart", "left")], col)


def p_shop():
    intro = wsec([column([
        paragraph("Original paintings and limited prints, shipped worldwide. "
                  "Every original is one of a kind.", BODY, "center", 16, 1.7),
    ])], 60, 20, WHITE, {"content_width": {"unit": "px", "size": 720,
                                           "sizes": []}})
    rows, row = [], []
    for i, (t, m, pr) in enumerate(WORKS):
        row.append(shop_item(art_ph(t.split()[0]), t, m, pr))
        if len(row) == 3:
            rows.append(section(row, {"gap": "wide"}, inner=True))
            rows.append(section([column([spacer(30)])], inner=True))
            row = []
    ...
    note = column([spacer(10), paragraph(
        "Prices are placeholders. Once WooCommerce is installed, replace these "
        "cards with the Products widget for real cart &amp; checkout — see "
        "SHOP-SETUP.md.", MUTED, "center", 12, 1.6)])
    body = wsec([column(rows + [note])], 30, 90)
    return [hero("Buy Art", "Shop", big=54, pad_top=80, pad_bot=20),
            intro, marker(), body]
...
def build_kit():
    kdir = os.path.join(os.path.dirname(__file__), "kit-build")
